...
function launchTest() {
  ...
  client._devices.forEach(async function (d) {    
    ...
    slide(1000, 0);
    testTimeout = setTimeout(() => stopTest(), 1000);
  });
}
...
function checkVibration(currentTimeInSeconds) {
    ...
    if (progression && progression.length > 0) {
      progression = progression[progression.length - 1].pos/100;
      client._devices.forEach(async function (d) {
        console.log("time:"+currentTimeInMilliseconds+", slide:"+progression);
        Slide(d, progression);
      });    
  }
}
...
function Slide(device, power) {
  client.SendDeviceMessage(device, device.SendLinearCmd(power));
}
